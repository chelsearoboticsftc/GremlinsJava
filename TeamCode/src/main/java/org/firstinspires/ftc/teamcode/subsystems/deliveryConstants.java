package org.firstinspires.ftc.teamcode.subsystems;

public class deliveryConstants {
    final public static double LINEARSLIDE_VEL_FF = 15.906;
    final public static double LINEARSLIDE_VEL_P = 1.5906;
    final public static double LINEARSLIDE_VEL_I = 0.15906;
    final public static double LINEARSLIDE_VEL_D = 0;
    final public static double LINEARSLIDE_POS_P = 5;
    final public static double LINEARSLIDE_POS_TOLERENCE = 5;

    final public static double LINEARSLIDE_SET_VELOCITY = 2240;
    final public static double MAX_LINEAR_SLIDE_VELOCITY = 1;
    final public static double LINEARSIDE_GO_POWER = 0.1;
    final public static double LINEARSLIDE_STOP_POWER = 0;

    final public static double ARM_VEL_FF = 15.906;
    final public static double ARM_VEL_P = 1.5906;
    final public static double ARM_VEL_I = 0.15906;
    final public static double ARM_VEL_D = 0;
    final public static double ARM_POS_P = 5;
    final public static double ARM_POS_TOLERENCE = 5;

    final public static double ARM_SET_VELOCITY = 2240;
    final public static double MAX_ARM_VELOCITY = 1;
    final public static double ARM_GO_POWER = 0.1;
    final public static double ARM_STOP_POWER = 0;

    final public static double CLAW_EAT_OPEN = 1;
    final public static double CLAW_EAT_CLOSE = 0.47;


    final public static int HIGH_SPECIMEN_BAR = -456;
    final public static int PUSH_DOWN_ON_SPECIMEN_HB = -446;

    //final public static int LOW_SPECIMEN_BAR = 150;
    //final public static int PUSH_DOWN_ON_SPECIMEN_LB = 140;

   // final public static int LOW_BASKET = 100;

    //final public static int HIGH_BASKET = 250;

    final public static int HOME = 0;

    final public static int ARM_HB_POSITION = -66;

    final public static int SPECIMEN_OFF_WALL_GRAB = 50;
    final public static int SPECIMAN_OFF_WALL_CLEAR = 60;

}
